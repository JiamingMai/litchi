...
public class HoltWintersModel {
    ...
    public Matrix optimize(Matrix y, BigDecimal cycleLen) {
        return optimize(y, cycleLen, DEFAULT_EPOCH_NUM);
    }

    public Matrix optimize(Matrix y, BigDecimal cycleLen, int epochNum) {
    ...
    class HoltWintersModelRmseTargetFunction implements TargetFunction {
        ...
        private BigDecimal cycleLen;

        public HoltWintersModelRmseTargetFunction(BigDecimal cycleLen) {
        ...
        private BigDecimal calcRmse(Matrix y, Matrix params) {
            ...
            // Initialize s0
            BigDecimal sum = new BigDecimal(0.0);
            for (int i = 0; i < cycleLen.intValue(); i++) {
                sum = sum.add(y.getValue(i, 0));
            }
            BigDecimal s0 = sum.multiply(new BigDecimal(1.0 / cycleLen.doubleValue()));
            s.add(s0);

            // Initialize b0
            sum = new BigDecimal(0.0);
            for (int i = cycleLen.intValue(); i < 2 * cycleLen.intValue(); i++) {
                sum = sum.add(y.getValue(i, 0).subtract(y.getValue(i - cycleLen.intValue(), 0)));
            }
            BigDecimal b0 = sum.multiply(new BigDecimal(1.0 / (cycleLen.doubleValue() * cycleLen.doubleValue())));
            b.add(b0);
