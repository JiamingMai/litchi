...
public class Problem1 {
    ...
    class AutoEncoderOutlierDetectionService extends OutlierDetectionService {
        ...
        private AutoEncoderModel autoEncoderModel = new AutoEncoderModel(2);
        ...
        public Map<String, Double[]> detectOutlier(Map<String, Double[]> records) {
            ...
            if (null != keys && !keys.isEmpty()) {
                ...
                if (featureNum > MAX_FEATURE_NUM) {
                    ...
                    autoEncoderModel.train(xiArray);
                    Matrix reducedX = new Matrix(xiArray.length, MAX_FEATURE_NUM);
                    for (int j = 0; j < xiArray.length; j++) {
                        Matrix xj = xiArray[j];
                        reducedX = AlgebraUtil.setRowVector(reducedX, j, AlgebraUtil.transpose(autoEncoderModel.encode(xj)));
                    }
                    x = reducedX;
                }
            } else {
