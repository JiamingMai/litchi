...
class MyTargetFunction(tf.TargetFunction):

    def fun(self, params, args):
        return params + 0

if __name__ == "__main__":
    target_function = MyTargetFunction()
    f = RmseFunction(target_function)
    params = np.array([3, 4, 7])
    args = np.array([[1,3], [2, 4], [5, 9]])
    rmse = f.fun(params, args)
    print(rmse)