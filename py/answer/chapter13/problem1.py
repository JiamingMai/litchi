...
import py.algorithm.auto_encoder_model as aem
import py.algebra.albebra_util as au

class OutlierDetectionServiceWithAutoEncoder():

    def __init__(self):
        self.max_feature_num = 2
        self.auto_encoder = aem.AutoEncoder()
        self.epsilon = 0.01
    ...
    def detect_outlier(self, records):
        ...
        # reduce dimension with auto-encoder model if there are too many features
        if feature_num > self.max_feature_num:
            ...
            for i in range(record_num):
                ...
                inputs.append(input)
            self.auto_encoder.train(inputs, self.max_feature_num)
            for i in range(record_num):
                xi = inputs[i]
                x[i, :] = np.transpose(self.auto_encoder.encode(xi))
