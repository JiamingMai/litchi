...
import py.algorithm.moving_average_model as moving_average_model
import numpy as np

class TrendLineService():

    def wrap_mat(self, values):
        x_mat = np.mat(np.zeros((len(values), 1)))
    ...
    def select_best_model(self, x, y):
        ...
        models.append(moving_average_model)
        best_model_index = -1
        min_rmse = 9999999
    ...
    def estimate_values(self, values, trend_line_type):
        ...
        elif trend_line_type == tle.TrendLineEnum.MOVING_AVERAGE:
            basic_model = moving_average_model
        else:
            # can't happen
            return
