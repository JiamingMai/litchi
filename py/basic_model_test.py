...
def compare_basic_model():
    ...
    logarithm_params = py.logarithm_model.optimize(x, y)
    logarithm_y_hat = py.logarithm_model.calc_value(x, logarithm_params)
    logarithm_rmse = calc_rmse(logarithm_y_hat, y)
    print("Logarithm Model RMSE: %.2f" %logarithm_rmse)

    power_params = py.power_model.optimize(x, y)
    power_y_hat = py.power_model.calc_value(x, power_params)
    power_rmse = calc_rmse(power_y_hat, y)
    print("Power Model RMSE: %.2f" %power_rmse)

    exponential_params = py.exponential_model.optimize(x, y)
    exponential_y_hat = py.exponential_model.calc_value(x, exponential_params)
    exponential_rmse = calc_rmse(exponential_y_hat, y)
    print("Exponential Model RMSE: %.2f" %exponential_rmse)

    polynomial_params = py.polynomial_model.optimize(x, y)
    polynomial_y_hat = py.polynomial_model.calc_value(x, polynomial_params)
